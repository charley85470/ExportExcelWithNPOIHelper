        ...
        public byte[] ExportToExcel(IEnumerable<T> list)
        {
            ...
            foreach (var prop in srcProps)
            {
                ...
                if (attr != null)   // 判斷該Prop是否有套用ExcelExport的Attribute，若有才將該欄位輸出至Excel
                {
                    ...
                    var column = ((ExcelExportAttribute)attr).Column == 0 ? defaultCol : ((ExcelExportAttribute)attr).Column - 1;
                ...
                foreach(var item in list)
                {
                    ...
                    foreach (var prop in props)
                    {
                        ...
                        if (attr != null)
                        {
                            ...
                            var column = ((ExcelExportAttribute)attr).Column == 0 ? defaultCol : ((ExcelExportAttribute)attr).Column;
